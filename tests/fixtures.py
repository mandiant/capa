...
def parse_feature_string(s: str) -> Feature:
    ...
    if key == "regex":
        return capa.features.common.Regex(f"/{raw_value}/")
...
def _load_feature_tests() -> tuple[list[tuple], list[tuple]]:
    with open(CD / "fixtures" / "feature-presence.json", "r") as f:
        data = json.load(f)
