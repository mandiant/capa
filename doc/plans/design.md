...
For source-file separators and HTML source grouping, mapa uses the first Assemblage record's normalized source path as the function's primary source path. The text renderer tracks the last seen non-empty primary path across the function list. Missing Assemblage data does not trigger a separator or any HTML source-query match and does not reset that state. When a later function introduces a different primary path, the renderer prints a muted horizontal rule with `[ <path> ]` immediately before that function.

`mapa/html_renderer.py` renders a single self-contained HTML document. It emits a split view: a left function pane and a right string pane, both with independent scrolling. The panes are separated by a draggable vertical divider implemented with a small inline pointer-event handler. The renderer emits one square per function in address order, one program-string row per string VA in address order, tag controls with visible function counts, a small legend for border/fill/source/dim semantics, a plain editable source-query input, right-aligned visible tags in each string row, inline JSON data for tag memberships, string memberships, function summaries, and exact-match primary-source-path groups, plus a single floating tooltip and a small inline script. That script keeps hover-based source grouping exact, copies a clicked function's source path into the input, treats an unchanged clicked path as an exact match, and switches to raw `startsWith()` prefix matching across primary source paths as soon as the user edits the input.
