...
_RE_CSHARP_PAGE = re.compile(rb'@ .*Page Language\s*=\s*"C#".*', re.IGNORECASE)
_RE_ASPX_IMPORT_DIRECTIVE = re.compile(rb"@\s*Import Namespace=", re.IGNORECASE)
_RE_ASPX_NAMESPACE = re.compile(rb'@\s*Import namespace="(.*?)"', re.IGNORECASE)
_RE_RUNAT_SERVER = re.compile(rb'runat\s*=\s*"server"')
