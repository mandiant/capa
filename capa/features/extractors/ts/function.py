...
def is_pseudo_main_function(fh: FunctionHandle, engine: TreeSitterBaseEngine) -> bool:
    return (
...
def extract_strings(fn_node: Node, engine: TreeSitterBaseEngine) -> Iterator[Tuple[Feature, Address]]:
...
def extract_integers(fn_node: Node, engine: TreeSitterBaseEngine) -> Iterator[Tuple[Feature, Address]]:
...
def _extract_static_methods(node: Node, engine: TreeSitterBaseEngine) -> Iterator[Tuple[Feature, Address]]:
...
def _extract_bash_function_calls(fn_node: Node, engine: TreeSitterBashEngine) -> Iterator[Tuple[Feature, Address]]:
    for node in engine.get_function_call_names(fn_node):
        yield from _extract_static_methods(node, engine)


def _extract_bash_pseudo_main_features(engine: TreeSitterBashEngine) -> Iterator[Tuple[Feature, Address]]:
    for node in engine.get_global_statements():
        yield from _extract_bash_features(node, engine)


def _extract_bash_features(fn_node: Node, engine: TreeSitterBashEngine) -> Iterator[Tuple[Feature, Address]]:
    for function_handler in BASH_FUNCTION_HANDLERS:
        for feature, addr in function_handler(fn_node, engine):
            yield feature, addr


@singledispatch
def _extract_engine_features(engine: TreeSitterBaseEngine, fh: FunctionHandle) -> Iterator[Tuple[Feature, Address]]:
    raise TypeError(f"unsupported Tree-Sitter engine: {type(engine).__name__}")


@_extract_engine_features.register
def _(engine: TreeSitterExtractorEngine, fh: FunctionHandle) -> Iterator[Tuple[Feature, Address]]:
...
@_extract_engine_features.register
def _(engine: TreeSitterBashEngine, fh: FunctionHandle) -> Iterator[Tuple[Feature, Address]]:
    if is_pseudo_main_function(fh, engine):
        yield from _extract_bash_pseudo_main_features(engine)
    else:
        yield from _extract_bash_features(fh.inner.node, engine)


def extract_features(fh: FunctionHandle, engine: TreeSitterBaseEngine) -> Iterator[Tuple[Feature, Address]]:
    yield from _extract_engine_features(engine, fh)
...
BASH_FUNCTION_HANDLERS = (
    _extract_bash_function_calls,
    extract_integers,
    extract_strings,
)
