...
def extract_file_embedded_pe(ctx=None) -> Iterator[Tuple[Feature, Address]]:
...
def extract_file_export_names(ctx=None) -> Iterator[Tuple[Feature, Address]]:
    """extract function exports"""
    for _, _, ea, name in idautils.Entries():
        yield Export(name), AbsoluteVirtualAddress(ea)


def extract_file_import_names(ctx=None) -> Iterator[Tuple[Feature, Address]]:
...
def extract_file_section_names(ctx=None) -> Iterator[Tuple[Feature, Address]]:
...
def extract_file_strings(ctx=None) -> Iterator[Tuple[Feature, Address]]:
    ...
    for seg in capa.features.extractors.ida.helpers.get_segments():
        ...
        for s in capa.features.extractors.strings.extract_ascii_strings(seg_buff, min_str_len=ctx["min_str_len"]):
            yield String(s.s), FileOffsetAddress(seg.start_ea + s.offset)

        for s in capa.features.extractors.strings.extract_unicode_strings(seg_buff, min_str_len=ctx["min_str_len"]):
            yield String(s.s), FileOffsetAddress(seg.start_ea + s.offset)


def extract_file_function_names(ctx=None) -> Iterator[Tuple[Feature, Address]]:
    """
...
def main():
    ...
    pprint.pprint(list(extract_features(ctx={"min_str_len": DEFAULT_STRING_LENGTH})))
