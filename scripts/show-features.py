...
def ida_main():
    ...
    print(f"getting features for current function 0x{function:X}")
    ...
    if function:
        ...
        if len(function_handles) == 0:
            print(f"0x{function:X} not a function")
            return -1
                ...
                for feature, addr in extractor.extract_insn_features(f, bb, insn):
                    ...
                    try:
                        if insn.address != addr:
                            print(f"  insn: {format_address(f.address)}: {format_address(insn.address)}: {feature} -> {format_address(addr)}")
                        else:
