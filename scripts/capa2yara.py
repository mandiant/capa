...
def convert_rule(rule, rulename, cround, depth):
    ...
    def do_statement(s_type, kid):
        ...
        if check_feature(s_type, rulename):
            ...
            yara_condition += '\tfor any ' + var_name_sec + ' in pe.sections : ( ' + var_name_sec + '.name == "' + section + '" ) '
            ...
            # chage capas /xxx/i to yaras /xxx/ nocase, count will be used later to decide appending 'nocase'
    ...
    if 'kids' not in locals().keys():
        ...
        for kid in kids:
            ...
            if s_type == 'And' or s_type == 'Or' or s_type == 'Not' and not kid.name == 'Some':
                ...
                yara_strings_sub, yara_condition_sub, rule_comment_sub, incomplete_sub = convert_rule(kid, rulename, cround, depth)

                logger.info("coming out of this recursion, depth: " + repr(depth) + ' s_type: ' + s_type)

                if yara_strings_sub == 'BREAK':
                    logger.info("Unknown feature at2: "+ rule.name + ' - s_type: ' + s_type + ' - depth: ' + str(depth) )
                    ...
                    if rule.name == 'Or' and depth == 1 and cround > min_rounds -1:
                        logger.info("Unknown feature, just ignore this branch and keep the rest bec we're in Or (1): "+ s_type + ' - depth: ' + str(depth) )
                        # remove last 'or'
            ...
            if yara_strings_sub == 'BREAK':
                ...
                logger.info("rule.name,  depth,  cround: " + rule.name + ', ' + str(depth) + ', ' + str( cround ))
                if rule.name == 'Or' and depth == 1 and cround > min_rounds -1:
                    logger.info("Unknown feature, just ignore this branch and keep the rest bec we're in Or (2): "+ s_type + ' - depth: ' + str(depth) )
    ...
    if not yara_condition_list:
        return 'BREAK', 'Multiple statements inside "- or:" where all unsupported, the last one was "' + s_type + '"', rule_comment, incomplete
...
def output_unsupported_capa_rules(yaml, capa_rulename, url, reason):
    ...
    if reason != 'NOLOG':
        if capa_rulename not in unsupported_capa_rules_list:
            ...
            unsupported_capa_rules.write(('Reason: '+ reason + ' (there might be multiple unsupported things in this rule, this is the 1st one encountered)').encode("utf-8") + b'\n')
...
def main(argv=None):
    ...
    output_yar('// Rules from Fireeyes https://github.com/fireeye/capa-rules converted to YARA using https://github.com/fireeye/capa/blob/master/scripts/capa2yara.py by Arnim Rupp')
    output_yar('// Beware: These are less rules than capa (because not all fit into YARA, stats at EOF) and is less precise because e.g. capas function scopes are applied to the whole file')
    output_yar('// Beware: Some rules are incomplete because an optional branch was not supported by YARA. These rules are marked in a comment in meta: (search for "incomplete")')
    output_yar('// Rule authors and license stay the same')
    output_yar('// att&ck and MBC tags are put into YARA rule tags. All rules are tagged with "CAPA" for easy filtering')
