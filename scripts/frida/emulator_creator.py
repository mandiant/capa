...
def get_system_architecture():
    ...
    logger.debug(f"Detected system: {platform.machine()}")
    ...
    if machine in ["arm64", "aarch64"]:
        ...
        raise RuntimeError("Unsupported architecture. Manual setup required.")
...
def setup_android_sdk_path():
    ...
    logger.debug("Add Android tools to PATH")
...
def create_emulator(avd_arch):
    ...
    if result.returncode != 0:
        raise RuntimeError(f"avdmanager failed: {result.stderr}")

    logger.info(f"Emulator '{avd_name}' created")
    return avd_name
...
def download_frida_server(frida_arch):
    ...
    if extracted_file.exists():
        logger.info("Using cached frida-server")
        return str(extracted_file)

    logger.info(f"Downloading frida-server for {frida_arch}...")
    url = (
    ...
    if shutil.which("curl"):
        ...
        raise RuntimeError(f"Neither curl nor wget found. Manual download required: {url}")
...
def setup_frida_server(frida_arch):
    ...
    logger.info("Installing frida-server to device...")
...
def create_frida_emulator():
    ...
    avd_arch, frida_arch = get_system_architecture()
